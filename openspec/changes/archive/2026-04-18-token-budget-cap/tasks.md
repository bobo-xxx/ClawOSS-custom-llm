...
- [x] 1.1 Add `BUDGET_CAP_USD` to `.env.example` (currently set to `1.0` for low-budget testing)
- [ ] 1.2 Add `DAILY_BUDGET_USD` to `.env.example` as optional override (commented out)

## 2. Dashboard Budget API Endpoint

- [x] 2.1 Create `dashboard/app/api/metrics/budget/route.ts` with GET handler
- [x] 2.2 Implement budget calculation logic (query metricsTokens, compute percentages)
- [x] 2.3 Handle edge cases: missing data, invalid cap values, database errors
- [x] 2.4 Add proper TypeScript types for budget response

## 3. HEARTBEAT Budget Check Integration

- [x] 3.1 Update `workspace/HEARTBEAT.md` step 0b to include budget check
- [x] 3.2 Add curl call to `/api/metrics/budget` endpoint
- [x] 3.3 Implement idle mode logic (skip steps 3-5 when budget exceeded)
- [x] 3.4 Add `BUDGET_EXCEEDED` logging to dashboard
- [x] 3.5 Handle API failure gracefully (fail-open with warning log)

## 4. Safety-Checker Budget Implementation

- [x] 4.1 Update `workspace/skills/safety-checker/SKILL.md` Check 1 with implementation details
- [ ] 4.2 Add logic to read `memory/daily-cost.md`
- [ ] 4.3 Implement daily allocation calculation from `BUDGET_CAP_USD`
- [x] 4.4 Add budget exceeded abort logic with proper result logging
...
- [x] 7.1 Verify budget endpoint returns correct values with test data
